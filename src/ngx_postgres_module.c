...
static void ngx_postgres_srv_conf_cleanup_handler(void *data) {
    ...
    queue_each(&usc->keep.queue, q) ngx_postgres_close(queue_data(q, ngx_postgres_save_t, queue));
    queue_each(&usc->work.queue, q) ngx_postgres_close(queue_data(q, ngx_postgres_save_t, queue));
}
...
static char *ngx_postgres_keepalive_conf(ngx_conf_t *cf, ngx_command_t *cmd, void *conf) {
    ...
    ngx_http_upstream_srv_conf_t *usc = ngx_http_conf_get_module_srv_conf(cf, ngx_http_upstream_module);
    if ((pusc->keep.max = (ngx_uint_t)n) < usc->servers->nelts) { ngx_conf_log_error(NGX_LOG_EMERG, cf, 0, "\"%V\" directive error: \"%V\" must be greater or equal than servers count (%i)", &cmd->name, &args[1], usc->servers->nelts); return NGX_CONF_ERROR; }
...
static char *ngx_postgres_pass_conf(ngx_conf_t *cf, ngx_command_t *cmd, void *conf) {
    ...
    ngx_http_upstream_srv_conf_t *usc = location->upstream.upstream;
    usc->peer.init_upstream = ngx_postgres_peer_init_upstream;
#if (T_NGX_HTTP_DYNAMIC_RESOLVE)
