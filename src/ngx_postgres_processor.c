...
static ngx_int_t ngx_postgres_error(ngx_postgres_data_t *d) {
    ...
    const char *value;
    ngx_postgres_save_t *s = d->save;
    if ((value = PQcmdStatus(s->res)) && ngx_strlen(value)) { ngx_log_error(NGX_LOG_WARN, s->connection->log, 0, "PQresultStatus == %s and %s and %s", PQresStatus(PQresultStatus(s->res)), value, PQresultErrorMessageMy(s->res)); }
    else { ngx_log_error(NGX_LOG_WARN, s->connection->log, 0, "PQresultStatus == %s and %s", PQresStatus(PQresultStatus(s->res)), PQresultErrorMessageMy(s->res)); }
...
#if (PG_VERSION_NUM >= 140000)
    if ((s->res = PQgetResult(s->conn)) && PQresultStatus(s->res) != PGRES_PIPELINE_SYNC) { ngx_log_error(NGX_LOG_ERR, s->connection->log, 0, "PQresultStatus != PGRES_PIPELINE_SYNC"); return NGX_ERROR; }
    if ((s->res = PQgetResult(s->conn))) { ngx_log_error(NGX_LOG_ERR, s->connection->log, 0, "PQgetResult"); return NGX_ERROR; }
    if (!PQexitPipelineMode(s->conn)) { ngx_log_error(NGX_LOG_ERR, s->connection->log, 0, "!PQexitPipelineMode and %s", PQerrorMessageMy(s->conn)); return NGX_ERROR; }
#endif
...
#if (PG_VERSION_NUM >= 140000)
    if (!PQpipelineSync(s->conn)) { ngx_log_error(NGX_LOG_ERR, s->connection->log, 0, "!PQpipelineSync and %s", PQerrorMessageMy(s->conn)); return NGX_ERROR; }
#endif
    return NGX_AGAIN;
}
...
static ngx_int_t ngx_postgres_send_deallocate_or_prepare_or_query(ngx_postgres_save_t *s) {
    ...
    while (PQstatus(s->conn) == CONNECTION_OK && (s->res = PQgetResult(s->conn))) {
        switch (PQresultStatus(s->res)) {
            ...
            case PGRES_PIPELINE_ABORTED:
#endif
            case PGRES_FATAL_ERROR: if (d->catch) { PQclear(s->res); return ngx_postgres_error(d); } ngx_log_debug2(NGX_LOG_DEBUG_HTTP, s->connection->log, 0, "PQresultStatus == %s and %s", PQresStatus(PQresultStatus(s->res)), PQresultErrorMessageMy(s->res)); break;
    ...
    for (ngx_uint_t i = 0; i < location->query.nelts; i++) {
        ...
        if (query->ids.nelts) {
            ...
            for (ngx_uint_t i = 0; i < query->ids.nelts; i++) {
                ...
                send->sql.len += ids[i].len;
            }
        }
        if (!(send->sql.data = ngx_pnalloc(r->pool, send->sql.len + 1))) { ngx_log_error(NGX_LOG_ERR, s->connection->log, 0, "!ngx_pnalloc"); return NGX_ERROR; }
        av_alist alist;
        u_char *last = NULL;
        av_start_ptr(alist, &ngx_snprintf, u_char *, &last);
        if (av_ptr(alist, u_char *, send->sql.data)) { ngx_log_error(NGX_LOG_ERR, s->connection->log, 0, "av_ptr"); return NGX_ERROR; }
        if (av_ulong(alist, send->sql.len)) { ngx_log_error(NGX_LOG_ERR, s->connection->log, 0, "av_ulong"); return NGX_ERROR; }
