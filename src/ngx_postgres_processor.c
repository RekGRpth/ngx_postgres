...
static ngx_int_t ngx_postgres_send_query(ngx_http_request_t *r) {
    ...
    if (!pd->stmtName) {
        if (pd->nParams) {
            ...
            if (!PQsendQuery(pd->common.conn, (const char *)pd->sql.data)) { ngx_log_error(NGX_LOG_ERR, r->connection->log, 0, "!PQsendQuery(%s) and %s", pd->sql.data, PQerrorMessageMy(pd->common.conn)); return NGX_ERROR; }
        }
...
again:
    ...
    if (charset) {
        ...
        } else if (pd->common.charset.len == sizeof("windows1251") - 1 && !ngx_strncasecmp((u_char *)charset, (u_char *)"windows1251", sizeof("windows1251") - 1)) {
            ngx_str_set(&pd->common.charset, "windows-1251");
        } else if (pd->common.charset.len == sizeof("koi8r") - 1 && !ngx_strncasecmp((u_char *)charset, (u_char *)"koi8r", sizeof("koi8r") - 1)) {
            ngx_str_set(&pd->common.charset, "koi8-r");
        } else {
...
static ngx_int_t ngx_postgres_get_result(ngx_http_request_t *r) {
    ...
    if (!PQconsumeInput(pd->common.conn)) { ngx_log_error(NGX_LOG_ERR, r->connection->log, 0, "!PQconsumeInput and %s", PQerrorMessageMy(pd->common.conn)); return NGX_ERROR; }
    if (PQisBusy(pd->common.conn)) { ngx_log_debug0(NGX_LOG_DEBUG_HTTP, r->connection->log, 0, "PQisBusy"); return NGX_AGAIN; }
    if (!(pd->result.res = PQgetResult(pd->common.conn))) { ngx_log_error(NGX_LOG_ERR, r->connection->log, 0, "!PQgetResult and %s", PQerrorMessageMy(pd->common.conn)); return NGX_ERROR; }
...
void ngx_postgres_process_events(ngx_http_request_t *r) {
    ...
    switch (pd->common.state) {
        ...
        default: ngx_log_error(NGX_LOG_ERR, r->connection->log, 0, "state == %i", pd->common.state); return ngx_postgres_next_upstream(r, NGX_HTTP_UPSTREAM_FT_ERROR);
    }
