    ...
    while (PQstatus(s->conn) == CONNECTION_OK && (pd->result.res = PQgetResult(s->conn))) {
        switch (PQresultStatus(pd->result.res)) {
            case PGRES_FATAL_ERROR:
                ngx_log_error(NGX_LOG_ERR, c->log, 0, "PQresultStatus == PGRES_FATAL_ERROR and %s", PQresultErrorMessageMy(pd->result.res));
            ...
            default:
                if ((value = PQcmdStatus(pd->result.res)) && ngx_strlen(value)) { ngx_log_debug2(NGX_LOG_DEBUG_HTTP, c->log, 0, "%s and %s", PQresStatus(PQresultStatus(pd->result.res)), value); }
                else { ngx_log_debug0(NGX_LOG_DEBUG_HTTP, c->log, 0, PQresStatus(PQresultStatus(pd->result.res))); }
                break;
        }
        PQclear(pd->result.res);
        switch (ngx_postgres_consume_flush_busy(s)) {
...
static ngx_int_t ngx_postgres_prepare_result(ngx_postgres_save_t *s) {
    ...
    while (PQstatus(s->conn) == CONNECTION_OK && (pd->result.res = PQgetResult(s->conn))) {
        switch (PQresultStatus(pd->result.res)) {
            ...
            default:
                ngx_log_error(NGX_LOG_ERR, c->log, 0, "PQresultStatus == %s and %s and %s", PQresStatus(PQresultStatus(pd->result.res)), PQcmdStatus(pd->result.res), PQresultErrorMessageMy(pd->result.res));
                ngx_postgres_variable_error(pd);
                PQclear(pd->result.res);
                return ngx_postgres_done(pd, NGX_HTTP_INTERNAL_SERVER_ERROR);
        }
    ...
    while (PQstatus(s->conn) == CONNECTION_OK && (pd->result.res = PQgetResult(s->conn))) {
        switch (PQresultStatus(pd->result.res)) {
            case PGRES_FATAL_ERROR:
                ...
                PQclear(pd->result.res);
                return ngx_postgres_done(pd, NGX_HTTP_INTERNAL_SERVER_ERROR);
            default: ngx_log_debug2(NGX_LOG_DEBUG_HTTP, c->log, 0, "PQresultStatus == %s and %s", PQresStatus(PQresultStatus(pd->result.res)), PQcmdStatus(pd->result.res)); break;
        }
        PQclear(pd->result.res);
        switch (ngx_postgres_consume_flush_busy(s)) {
            case NGX_AGAIN: return NGX_AGAIN;
...
static ngx_int_t ngx_postgres_prepare(ngx_postgres_save_t *s) {
    ...
    ngx_postgres_upstream_srv_conf_t *usc = s->usc;
    if (usc->prepare.deallocate && queue_size(&s->prepare->queue) >= usc->prepare.max) return ngx_postgres_deallocate(s);
