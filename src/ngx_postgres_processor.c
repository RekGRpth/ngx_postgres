...
ngx_int_t ngx_postgres_prepare_or_query(ngx_postgres_data_t *pd) {
    ...
    switch (ngx_postgres_consume_flush_busy(c, pd->conn)) {
    ...
    if (query->ids.nelts) {
        ...
        for (ngx_uint_t i = 0; i < query->ids.nelts; i++) {
            ...
            if (!value || !value->data || !value->len) { ngx_str_set(&ids[i], "NULL"); } else {
                char *str = PQescapeIdentifier(pd->conn, (const char *)value->data, value->len);
                if (!str) { ngx_log_error(NGX_LOG_ERR, r->connection->log, 0, "!PQescapeIdentifier(%*.*s) and %s", value->len, value->len, value->data, PQerrorMessageMy(pd->conn)); return NGX_ERROR; }
...
static ngx_int_t ngx_postgres_query_result(ngx_postgres_data_t *pd) {
    ...
    while (PQstatus(pd->conn) == CONNECTION_OK && (pd->result.res = PQgetResult(pd->conn))) {
        ...
        switch (ngx_postgres_consume_flush_busy(pd->connection, pd->conn)) {
            case NGX_AGAIN: return NGX_AGAIN;
            case NGX_ERROR: return NGX_ERROR;
            default: break;
        }
    ...
    if (rc == NGX_OK && PQtransactionStatus(pd->conn) != PQTRANS_IDLE) {
...
static ngx_int_t ngx_postgres_result(ngx_postgres_data_t *pd) {
    ...
    ngx_postgres_query_t *query = &queryelts[pd->index];
    ngx_postgres_output_t *output = &query->output;
    if (output->handler == ngx_postgres_output_plain || output->handler == ngx_postgres_output_csv) if (output->single && !PQsetSingleRowMode(pd->conn)) ngx_log_error(NGX_LOG_WARN, r->connection->log, 0, "!PQsetSingleRowMode and %s", PQerrorMessageMy(pd->conn));
    pd->handler = ngx_postgres_query_result;
    return NGX_AGAIN;
}
...
static ngx_int_t ngx_postgres_query_prepared(ngx_postgres_data_t *pd) {
    ...
    ngx_postgres_send_t *sendelts = pd->send.elts;
    ngx_postgres_send_t *send = &sendelts[pd->index];
    if (!PQsendQueryPrepared(pd->conn, (const char *)send->stmtName.data, send->nParams, (const char *const *)send->paramValues, NULL, NULL, send->binary)) { ngx_log_error(NGX_LOG_ERR, r->connection->log, 0, "!PQsendQueryPrepared(\"%V\", \"%V\", %i) and %s", &send->stmtName, &send->sql, send->nParams, PQerrorMessageMy(pd->conn)); return NGX_ERROR; }
...
static ngx_int_t ngx_postgres_query(ngx_postgres_data_t *pd) {
    ...
    if (send->nParams || send->binary) {
        if (!PQsendQueryParams(pd->conn, (const char *)send->sql.data, send->nParams, send->paramTypes, (const char *const *)send->paramValues, NULL, NULL, send->binary)) { ngx_log_error(NGX_LOG_ERR, r->connection->log, 0, "!PQsendQueryParams(\"%V\", %i) and %s", &send->sql, send->nParams, PQerrorMessageMy(pd->conn)); return NGX_ERROR; }
        ngx_log_debug2(NGX_LOG_DEBUG_HTTP, r->connection->log, 0, "PQsendQueryParams(\"%V\", %i)", &send->sql, send->nParams);
    } else {
        if (!PQsendQuery(pd->conn, (const char *)send->sql.data)) { ngx_log_error(NGX_LOG_ERR, r->connection->log, 0, "!PQsendQuery(\"%V\") and %s", &send->sql, PQerrorMessageMy(pd->conn)); return NGX_ERROR; }
...
static ngx_int_t ngx_postgres_deallocate(ngx_postgres_data_t *pd) {
    ...
    ngx_postgres_send_t *sendelts = pd->send.elts;
    ngx_postgres_send_t *send = &sendelts[pd->index];
    char *str = PQescapeIdentifier(pd->conn, (const char *)send->stmtName.data, send->stmtName.len);
    if (!str) { ngx_log_error(NGX_LOG_ERR, r->connection->log, 0, "!PQescapeIdentifier(\"%V\") and %s", &send->stmtName, PQerrorMessageMy(pd->conn)); return NGX_ERROR; }
    ngx_str_t id = {ngx_strlen(str), NULL};
    ...
    *last = '\0';
    if (!PQsendQuery(pd->conn, (const char *)sql.data)) { ngx_log_error(NGX_LOG_ERR, r->connection->log, 0, "!PQsendQuery(\"%V\") and %s", &sql, PQerrorMessageMy(pd->conn)); return NGX_ERROR; }
    ngx_log_debug1(NGX_LOG_DEBUG_HTTP, r->connection->log, 0, "PQsendQuery(\"%V\")", &sql);
    ngx_queue_t *queue = ngx_queue_head(&pd->prepare->head);
...
static ngx_int_t ngx_postgres_prepare(ngx_postgres_data_t *pd) {
    ...
    ngx_postgres_upstream_srv_conf_t *pusc = pd->pusc;
    if (pd->prepare->size >= pusc->prepare.max && pusc->prepare.deallocate) return ngx_postgres_deallocate(pd);
    if (!PQsendPrepare(pd->conn, (const char *)send->stmtName.data, (const char *)send->sql.data, send->nParams, send->paramTypes)) { ngx_log_error(NGX_LOG_ERR, r->connection->log, 0, "!PQsendPrepare(\"%V\", \"%V\") and %s", &send->stmtName, &send->sql, PQerrorMessageMy(pd->conn)); return NGX_ERROR; }
    ...
    ngx_queue_insert_tail(&pd->prepare->head, &prepare->item);
    pd->prepare->size++;
    pd->handler = ngx_postgres_prepare_result;
    return NGX_AGAIN;
}
...
ngx_int_t ngx_postgres_connect(ngx_postgres_data_t *pd) {
    ...
    ngx_postgres_upstream_srv_conf_t *pusc = pd->pusc;
    switch (PQstatus(pd->conn)) {
        case CONNECTION_BAD: ngx_log_error(NGX_LOG_ERR, r->connection->log, 0, "PQstatus == CONNECTION_BAD and %s", PQerrorMessageMy(pd->conn)); ngx_postgres_close(c, pd->conn, pusc); return NGX_ERROR;
        case CONNECTION_OK: ngx_log_debug0(NGX_LOG_DEBUG_HTTP, r->connection->log, 0, "PQstatus == CONNECTION_OK"); goto connected;
        default: break;
    }
again:
    switch (PQconnectPoll(pd->conn)) {
        case PGRES_POLLING_ACTIVE: ngx_log_debug1(NGX_LOG_DEBUG_HTTP, r->connection->log, 0, "PGRES_POLLING_ACTIVE and %s", ngx_postgres_status(pd->conn)); return NGX_AGAIN;
        case PGRES_POLLING_FAILED: ngx_log_error(NGX_LOG_ERR, r->connection->log, 0, "PGRES_POLLING_FAILED and %s and %s", ngx_postgres_status(pd->conn), PQerrorMessageMy(pd->conn)); ngx_postgres_close(c, pd->conn, pusc); return NGX_ERROR;
        case PGRES_POLLING_OK: ngx_log_debug1(NGX_LOG_DEBUG_HTTP, r->connection->log, 0, "PGRES_POLLING_OK and %s", ngx_postgres_status(pd->conn)); goto connected;
        case PGRES_POLLING_READING: ngx_log_debug1(NGX_LOG_DEBUG_HTTP, r->connection->log, 0, "PGRES_POLLING_READING and %s", ngx_postgres_status(pd->conn)); return NGX_AGAIN;
        case PGRES_POLLING_WRITING: ngx_log_debug1(NGX_LOG_DEBUG_HTTP, r->connection->log, 0, "PGRES_POLLING_WRITING and %s", ngx_postgres_status(pd->conn)); if (PQstatus(pd->conn) == CONNECTION_MADE) goto again; return NGX_AGAIN;
    }
connected:
