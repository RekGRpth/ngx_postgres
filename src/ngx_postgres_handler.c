...
static void ngx_postgres_finalize_request(ngx_http_request_t *r, ngx_int_t rc) {
    ngx_log_debug1(NGX_LOG_DEBUG_HTTP, r->connection->log, 0, "rc = %i", __func__, rc);
...
ngx_int_t ngx_postgres_handler(ngx_http_request_t *r) {
    ...
    if (!r->upstream->peer.connection) return NGX_AGAIN;
...
void ngx_postgres_finalize_upstream(ngx_http_request_t *r, ngx_http_upstream_t *u, ngx_int_t rc) {
    ...
    if (u->pipe && u->pipe->temp_file) { ngx_log_debug1(NGX_LOG_DEBUG_HTTP, r->connection->log, 0, "fd = %i", u->pipe->temp_file->file.fd); }
