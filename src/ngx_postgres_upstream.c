...
static ngx_int_t ngx_postgres_peer_get(ngx_peer_connection_t *pc, void *data) {
    ...
    ngx_queue_t *queue = ngx_queue_head(&common->server->peer);
    ngx_postgres_peer_t *peer = ngx_queue_data(queue, ngx_postgres_peer_t, queue);
    ngx_queue_remove(&peer->queue);
    ngx_queue_insert_tail(&common->server->peer, &peer->queue);
//    pc->cached = 0;
    ...
    if (/*common->server->save >= */common->server->nsave && ngx_queue_empty(&common->server->free)) {
#ifdef NGX_YIELD
...
//    common->server->save++; /* take spot in keepalive connection pool */
    int fd;
    if ((fd = PQsocket(common->conn)) == -1) { ngx_log_error(NGX_LOG_ERR, r->connection->log, 0, "PQsocket == -1"); goto invalid; }
    ngx_connection_t *c;
    if (!(c = common->connection = ngx_get_connection(fd, pc->log))) { ngx_log_error(NGX_LOG_ERR, r->connection->log, 0, "!ngx_get_connection"); goto invalid; }
    c->log_error = pc->log_error;
    c->log = pc->log;
...
//                    ngx_log_debug2(NGX_LOG_DEBUG_HTTP, c->log, 0, "channel = %V, command = %V", &listen->channel, &listen->command);
...
static u_char *ngx_postgres_listen(ngx_postgres_data_t *pd, ngx_postgres_save_t *ps) {
    ...
    if (common->listen) for (ngx_queue_t *queue_pd = ngx_queue_head(common->listen); queue_pd != ngx_queue_sentinel(common->listen); queue_pd = ngx_queue_next(queue_pd)) {
        ...
        if (!(listen->channel.data = ngx_pstrdup(ps->common.connection->pool, &listen_pd->channel))) { ngx_log_error(NGX_LOG_ERR, c->log, 0, "!ngx_pstrdup"); return NULL; }
        listen->channel.len = listen_pd->channel.len;
        if (!(listen->command.data = ngx_pstrdup(ps->common.connection->pool, &listen_pd->command))) { ngx_log_error(NGX_LOG_ERR, c->log, 0, "!ngx_pstrdup"); return NULL; }
...
static void ngx_postgres_free_to_save(ngx_postgres_data_t *pd, ngx_postgres_save_t *ps) {
    ...
    c->data = ps;
    c->idle = 1;
    c->log = common->server->log ? common->server->log : ngx_cycle->log;
    if (c->pool) c->pool->log = c->log;
    ngx_add_timer(c->read, common->server->timeout);
    c->read->delayed = 0;
...
static void ngx_postgres_free_peer(ngx_postgres_data_t *pd) {
    ...
    ngx_postgres_common_t *common = &pd->common;
//    if (!common) { ngx_log_error(NGX_LOG_WARN, r->connection->log, 0, "!common"); return; }
    ngx_connection_t *c = common->connection;
