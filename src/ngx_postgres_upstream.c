...
static void ngx_postgres_free_peer(ngx_peer_connection_t *pc, void *data) {
    ...
    switch (PQtransactionStatus(pd->conn)) {
        ...
        default: ngx_log_error(NGX_LOG_WARN, pc->log, 0, "PQtransactionStatus != PQTRANS_IDLE"); if (!PQrequestCancel(pd->conn)) { ngx_log_error(NGX_LOG_ERR, pc->log, 0, "!PQrequestCancel and %s", PQerrorMessageMy(pd->conn)); goto create; } break;
    }
#if (T_NGX_HTTP_DYNAMIC_RESOLVE)
    ...
    return;
create:
    if (!(ps = ngx_postgres_save_create(c, pd->conn, usc))) return;
    ngx_postgres_save_close(ps);
    pc->connection = NULL;
}
...
ngx_int_t ngx_postgres_peer_init(ngx_http_request_t *r, ngx_http_upstream_srv_conf_t *usc) {
    ...
    if (pd->usc->peer.init(r, usc) != NGX_OK) { ngx_log_error(NGX_LOG_ERR, r->connection->log, 0, "peer.init != NGX_OK"); return NGX_ERROR; }
    pd->request = r;
    ngx_http_upstream_t *u = r->upstream;
    pd->peer = u->peer;
    u->peer.data = pd;
