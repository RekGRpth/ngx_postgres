...
static void ngx_postgres_save_close(ngx_postgres_save_t *s) {
    ...
    c->read->active = 0;
    c->write->active = 1;
    if (!ngx_terminate && !ngx_exiting && ngx_http_push_stream_delete_channel_my && s->usc && s->usc->keep.max && PQstatus(s->conn) == CONNECTION_OK && s->write_handler(s) != NGX_ERROR) return;
    ngx_postgres_close(s);
}
...
static ngx_int_t ngx_postgres_open(ngx_peer_connection_t *pc, void *data) {
    ...
    ngx_http_request_t *r = d->request;
    ngx_http_upstream_t *u = r->upstream;
    ngx_postgres_upstream_srv_conf_t *pusc = u->conf->upstream->srv_conf ? ngx_http_conf_upstream_srv_conf(u->conf->upstream, ngx_postgres_module) : NULL;
#if (T_NGX_HTTP_DYNAMIC_RESOLVE)
