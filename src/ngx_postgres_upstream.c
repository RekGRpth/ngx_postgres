...
//    pc->cached = 0;
    ...
    if (server->max_save) {
        ngx_log_debug0(NGX_LOG_DEBUG_HTTP, r->connection->log, 0, "max_save");
        ...
        } else if (!server->reject && !server->max_data) {
            ngx_log_debug0(NGX_LOG_DEBUG_HTTP, r->connection->log, 0, "!reject and !max_data");
        } else {
...
invalid:
    ngx_postgres_free_connection(pdc, 0);
    return NGX_ERROR;
}
...
static u_char *ngx_postgres_listen(ngx_postgres_data_t *pd, ngx_postgres_save_t *ps) {
    ...
    if (pdc->listen) while (!ngx_queue_empty(pdc->listen)) {
        ngx_queue_t *queue_pd = ngx_queue_head(pdc->listen);
        ngx_postgres_listen_t *pdl = ngx_queue_data(queue_pd, ngx_postgres_listen_t, queue);
        ngx_postgres_common_t *psc = &ps->common;
        if (!psc->listen) {
        ...
        for (ngx_queue_t *queue_ps = ngx_queue_head(psc->listen); queue_ps != ngx_queue_sentinel(psc->listen); queue_ps = ngx_queue_next(queue_ps)) {
            ngx_postgres_listen_t *psl = ngx_queue_data(queue_ps, ngx_postgres_listen_t, queue);
...
static void ngx_postgres_save_timeout(ngx_event_t *ev) {
    ngx_log_debug1(NGX_LOG_DEBUG_HTTP, ev->log, 0, "%s", __func__);
    ngx_connection_t *c = ev->data;
    ngx_postgres_save_t *ps = c->data;
    ngx_postgres_common_t *psc = &ps->common;
...
static void ngx_postgres_free_to_save(ngx_postgres_data_t *pd, ngx_postgres_save_t *ps) {
    ...
    ngx_add_timer(&ps->timeout, server->keepalive);
}
...
//    if (c->write->active && ngx_event_flags & NGX_USE_LEVEL_EVENT && ngx_del_event(c->write, NGX_WRITE_EVENT, 0) != NGX_OK) { ngx_log_error(NGX_LOG_ERR, r->connection->log, 0, "ngx_del_event != NGX_OK"); return; }
    ...
    if (ngx_queue_empty(&server->free)) {
        ...
        ngx_postgres_common_t *psc = &ps->common;
        ngx_postgres_free_connection(psc, 1);
    } else {
...
static ngx_conf_enum_t ngx_postgres_oids[] = {
