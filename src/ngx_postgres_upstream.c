...
//    (void)ngx_cpystrn(value, addr.data + (pc->sockaddr->sa_family == AF_UNIX ? 5 : 0), addr.len + 1 + (pc->sockaddr->sa_family == AF_UNIX ? -5 : 0));
    ...
    int arg = 0;
    for (const char **keywords = connect->keywords, **values = connect->values; keywords && values; keywords++, values++, arg++) {
        ngx_log_debug3(NGX_LOG_DEBUG_HTTP, r->connection->log, 0, "%i: %s = %s", arg, *keywords ? *keywords : "(null)", *values ? *values : "(null)");
    }
