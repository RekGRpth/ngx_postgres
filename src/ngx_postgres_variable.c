...
ngx_int_t ngx_postgres_variable_set(ngx_http_request_t *r) {
    ...
    for (ngx_uint_t i = 0; i < variables->nelts; i++) {
        ngx_log_debug4(NGX_LOG_DEBUG_HTTP, r->connection->log, 0, "col = %i, row = %i, name = %s, required = %s", variable[i].col, variable[i].row, variable[i].name, variable[i].required ? "true" : "false");
